#!/usr/bin/env python3
"""
This file generates clos-style fat-trees in dot notation according to:
"A Scalable Commodity Data Center Network Architecture"
by Al-Fares, Loukissas, and Vahdat.
...
def mk_topo(pods, bw='1Gbps'):
    ...
    if num_agg_switches > 100:
        raise Exception("Naming convention doesn't work with more than 100 aggregation switches.")

    hosts = [('h' + str(i), {}) for i in range(1, num_hosts + 1)]

    agg_switches = [
        ('s' + str(i), {'type': 'switch', 'level': 'aggregation', 'id': i})
        for i in range(200, num_agg_switches + 200)
    ]

    core_switches = [
        ('s' + str(i), {'type': 'switch', 'level': 'core', 'id': i})
    ...
    for pod in range(pods):
        for sw in range(pods // 2):
            ...
            agg_switches[(pod * pods) + sw] = (
                "s" + str(edge_num),
                agg_switches[(pod * pods) + sw][1]
            )
            edge_num += 1

    edge_switches = list(filter(lambda sw: sw[1]['level'] == 'edge', agg_switches))
    agg_only_switches = list(filter(lambda sw: sw[1]['level'] != 'edge', agg_switches))
    ...
    a.add_subgraph([a for a, _ in hosts], rank='same')
    a.add_subgraph([a for a, _ in core_switches], rank='same')
    a.add_subgraph([a for a, _ in agg_only_switches], rank='same')
    a.add_subgraph([a for a, _ in edge_switches], rank='same')
...
def parse_args():
    ...
    parser.add_argument('-p', '--pods', type=int, default=4,
                        help='number of pods (parameter k in the paper)')
    parser.add_argument('-b', '--bandwidth', type=str, default='1Gbps',
...
if __name__ == '__main__':
    args = parse_args()
    topo = mk_topo(args.pods, args.bandwidth)
    ...
    if args.output:
        ...
        hosts = [n for n in topo.nodes() if n.startswith('h')]
        sws = [n for n in topo.nodes() if n.startswith('s')]
        switches = {switch: {} for switch in sws}
